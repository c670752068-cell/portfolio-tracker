...
function Row({ metric, onUpdate, onDelete, displayCurrency, rates }: RowProps) {
  ...
  return (
    <tr className="align-top hover:bg-slate-50 dark:hover:bg-slate-700/40">
      <td className="px-3 py-2"><div className="font-medium">{holding.symbol}</div><div className="text-xs text-slate-500">{label} · {holding.currency}</div>{holding.broker && <span className="mt-1 inline-block rounded bg-indigo-50 px-1.5 py-0.5 text-[10px] font-semibold text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-200">{brokerLabel(holding.broker)}</span>}{holding.assetType === 'leveraged_etf' && <label className="mt-1 block text-xs text-slate-500">杠杆倍数 <input type="number" min={0.5} max={5} step={0.1} value={holding.leverageFactor ?? ''} placeholder={String(leverageFactorFor(holding))} onChange={(event) => onUpdate(holding.id, { leverageFactor: event.target.value === '' ? undefined : Number(event.target.value) })} className="ml-1 w-14 rounded border border-slate-300 bg-transparent px-1 py-0.5 text-right dark:border-slate-600" /></label>}{optionDescription && <div className="mt-1 max-w-44 text-xs text-slate-500">{optionDescription}</div>}</td>
      ...
      <td className={`px-3 py-2 text-right tabular-nums ${dayClass}`}>{holding.quote?.change != null ? <><div>{formatDisplayMoney(metric.dayChange, displayCurrency, rates)}</div><div className="text-xs">{metric.dayChangePct != null ? formatSignedPct(metric.dayChangePct) : '涨跌率待补'}</div><div className="text-xs text-slate-400">{quoteSourceLabel(holding.quote.source)}</div></> : <div className="text-xs text-slate-400">未同步</div>}</td>
      <td className="px-3 py-2 text-right tabular-nums"><div>{formatDisplayMoney(metric.marketValue, displayCurrency, rates)}</div><div className="text-xs text-slate-500">{formatPct(metric.weight)}</div></td>
      <td className={`px-3 py-2 text-right tabular-nums ${pnlClass}`}>
        ...
        {metric.costKnown ? <><div>{formatDisplayMoney(metric.pnl, displayCurrency, rates)}</div><div className="text-xs">{formatSignedPct(metric.pnlPct)}</div></> : <div className="text-xs text-amber-600">成本待补</div>}
      </td>
      <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-300">{metric.deltaEquivalentShares != null ? <><div>{metric.deltaEquivalentShares.toFixed(1)} 股</div><div className="mt-1">Δ 暴露 ≈ {metric.deltaAdjustedExposure != null ? formatDisplayMoney(metric.deltaAdjustedExposure, displayCurrency, rates) : '待标的现价'}</div></> : '—'}</td>
