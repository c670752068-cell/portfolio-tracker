...
function VerdictCard({ verdict, asOf }: { verdict: QuantRegimeVerdictCard; asOf: string }) {
  ...
  return <article className="rounded-2xl border border-buy/35 bg-gradient-to-br from-buy/12 via-surface-raised to-surface-raised p-4 sm:p-6">
    <div className="flex items-center justify-between gap-3"><h3 className="text-label text-xs font-semibold uppercase text-buy">当前状态</h3><span className="text-label text-[11px] tabular-nums text-ink-muted">{dateText(asOf)}</span></div>
    <h4 className="text-display mt-3 text-xl font-semibold sm:text-2xl">{verdict.headline}</h4><p className="mt-1 text-sm text-ink-secondary">{verdict.cell_label}</p>
    {verdict.sample_sufficient ? <>
      ...
      <p className="mt-3 text-xs tabular-nums text-ink-muted">{verdict.sample_context}</p>
      {typeof tqqq === 'object' && tqqq && <p className="mt-3 rounded-xl bg-surface-overlay/45 px-3 py-2 text-xs tabular-nums text-ink-secondary">TQQQ 同状态 60 日：胜率 {percent(tqqq.win_rate_pct)} · 中位 {signed(tqqq.median_return_pct)} · n={tqqq.n}</p>}
      <p className="mt-3 rounded-xl border border-trim/35 bg-trim/10 px-3 py-2 text-sm font-semibold tabular-nums text-ink-primary">⚠ 最差情形：{worstCase}</p>
...
function Horizon({ days, item, emphasis }: { days: number; item: QuantRegimeVerdictCard['horizons'][string]; emphasis: boolean }) {
  if (!item) return <div className="rounded-xl bg-surface-overlay/45 p-3"><div className="text-xs text-ink-muted">{days} 日</div><div className="mt-2 text-sm tabular-nums text-ink-muted">暂无</div></div>;
  return <div className={`rounded-xl border p-3 ${emphasis ? 'border-buy/45 bg-buy/10' : 'border-neutral/30 bg-surface-overlay/45'}`}><div className="text-xs text-ink-muted">{days} 日</div><div className={`mt-2 font-semibold tabular-nums ${emphasis ? 'text-2xl text-buy sm:text-3xl' : 'text-lg text-ink-primary'}`}>{percent(item.win_rate_pct)}</div><div className="mt-1 text-xs tabular-nums text-ink-secondary">中位 {signed(item.median_return_pct)} · n={item.n}</div></div>;
}
...
function RegimeMatrix({ regime }: { regime: QuantRegimeStatus }) {
  ...
  return <details className="group rounded-2xl border border-neutral/40 bg-surface-raised"><summary className="flex cursor-pointer list-none items-center justify-between px-4 py-4 sm:px-5"><span><strong>查看完整状态矩阵</strong><span className="mt-1 block text-xs text-ink-muted">当前状态已在上方结论卡显示，此表用于对比其它状态。</span></span><svg viewBox="0 0 20 20" fill="none" aria-hidden="true" className="h-5 w-5 shrink-0 text-ink-muted transition-transform duration-200 ease-out group-open:rotate-180"><path d="m5 7.5 5 5 5-5" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" /></svg></summary><div className="border-t border-neutral/25 p-4 sm:p-5"><div className="grid grid-cols-[4.25rem_repeat(5,minmax(0,1fr))] gap-1 text-center sm:grid-cols-[5.5rem_repeat(5,minmax(0,1fr))]"><span />{cols.map((col) => <span key={col.index} className="text-[9px] tabular-nums text-ink-muted">{col.label}</span>)}{rows.flatMap((row) => [<span key={`r-${row.index}`} className="self-center text-left text-[10px] tabular-nums text-ink-muted">{row.label}</span>, ...cols.map((col) => { const cell = cells.find((item) => item.row_bucket_index === row.index && item.col_bucket_index === col.index); return <MatrixCell key={`${row.index}-${col.index}`} cell={cell} current={row.index === current.row_index && col.index === current.col_index} />; })])}</div></div></details>;
}

function MatrixCell({ cell, current }: { cell: QuantRegimeGridCell | undefined; current: boolean }) { const stat = cell?.reference; const sufficient = stat?.sample_sufficient; return <div className={`relative rounded-lg border px-0.5 py-2 ${current ? 'border-buy ring-2 ring-buy/35' : 'border-neutral/30'} ${sufficient ? 'bg-buy/10' : 'bg-surface-overlay/45 text-ink-muted'}`}><div className="text-[9px] font-semibold tabular-nums">{sufficient ? percent(stat?.win_rate_pct) : '样本不足'}</div><div className="text-[8px] tabular-nums">n={stat?.n ?? 0}</div>{cell && !cell.bear_included && <div className="mt-1 text-[8px] text-trim">仅牛市样本</div>}</div>; }
...
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-xl bg-surface-overlay/45 p-3"><div className="text-xs text-ink-muted">{label}</div><div className="mt-1 text-xl font-semibold tabular-nums text-ink-primary">{value}</div></div>; }
