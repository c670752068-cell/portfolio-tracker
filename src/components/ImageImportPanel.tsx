...
export function ImageImportPanel({ settings, onConfirm, onOptionDetails, onOpenSettings }: ImageImportPanelProps) {
  ...
  return (
    <div className="space-y-3 rounded-xl border border-buy/40 bg-buy/10 p-3 dark:border-buy/70 dark:bg-buy/20">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h2 className="text-sm font-semibold text-buy dark:text-buy">截图导入持仓</h2>
          <p className="mt-1 text-xs leading-relaxed text-ink-secondary dark:text-ink-secondary">
            完整持仓页请选择“全量持仓”；单独上传期权详情页请选择“补充期权详情”，该模式绝不会删除正股、ETF 或现金。
          </p>
        </div>
        <button onClick={onOpenSettings} className="text-xs font-medium text-buy hover:underline dark:text-buy">
          配置 AI Key
        </button>
      </div>
      ...
      {files.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {files.map((file, index) => (
            <span key={`${file.name}-${file.size}`} className="inline-flex max-w-full items-center gap-1 rounded-full bg-surface-raised px-2 py-1 text-xs text-ink-primary shadow-sm dark:bg-surface-raised dark:text-ink-secondary">
              <span className="truncate">{file.name}</span>
              <button aria-label={`移除 ${file.name}`} onClick={() => { setFiles(files.filter((_, itemIndex) => itemIndex !== index)); setResult(null); setOptionResult(null); setPrepareSummary(''); }} className="font-bold text-ink-muted hover:text-loss">×</button>
            </span>
          ))}
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button onClick={() => parse('full')} disabled={loadingMode !== null || files.length === 0} className="rounded-md bg-buy px-3 py-1.5 text-sm font-medium text-surface-base hover:bg-buy disabled:cursor-not-allowed disabled:bg-neutral">
          {loadingMode === 'full' ? '正在识别全量持仓…' : '解析并导入（全量持仓）'}
        </button>
        <button onClick={() => parse('option')} disabled={loadingMode !== null || files.length === 0} className="rounded-md border border-buy bg-surface-raised px-3 py-1.5 text-sm font-medium text-buy hover:bg-buy/10 disabled:cursor-not-allowed disabled:border-neutral disabled:text-ink-muted dark:bg-surface-base dark:text-buy">
      ...
      {result && (
        <div className="space-y-3 rounded-lg border border-buy/40 bg-surface-raised p-3 dark:border-buy dark:bg-surface-base">
          <div>
            <h3 className="text-sm font-semibold">识别预览</h3>
            <p className="mt-1 text-xs text-ink-muted">{result.sourceSummary}。已自动导入；如需修正，可在持仓表中编辑或删除。</p>
          </div>
          {result.holdings.length > 0 && (
            <div className="overflow-x-auto rounded border border-neutral/40 dark:border-neutral/60">
              <table className="min-w-full text-left text-xs">
                <thead className="bg-surface-overlay/30 text-ink-muted dark:bg-surface-raised dark:text-ink-muted">
                ...
                <tbody className="divide-y divide-neutral/30 dark:divide-neutral/40">
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setResult(null)} className="rounded-md bg-surface-overlay px-3 py-1.5 text-sm text-ink-primary hover:bg-neutral/40 dark:bg-surface-raised dark:text-ink-primary">关闭预览</button>
          </div>
        </div>
      )}

      {optionResult && (
        <div className="space-y-2 rounded-lg border border-gain/40 bg-surface-raised p-3 text-xs dark:border-gain/60 dark:bg-surface-base">
