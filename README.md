...
Moonshot API 默认对浏览器跨域请求不友好。如果点击「解析并预览」或「调用 Kimi 分析」报 `Load failed` / 网络错误，说明请求大概率还没到 Kimi，常见原因是 CORS、网络拦截或图片载荷过大。应用会先压缩截图；如果仍失败，按以下步骤部署一个 Cloudflare Worker 代理（5 分钟，免费）：
