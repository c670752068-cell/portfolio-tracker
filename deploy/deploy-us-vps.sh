...
ssh "${SERVER}" "
  ...
  (pm2 describe portfolio-tracker >/dev/null 2>&1 \
    && ${IMPORT_ARCHIVE_ENV}PORT=8788 HOST=0.0.0.0 STATIC_ROOT=/opt/portfolio-tracker-web pm2 restart portfolio-tracker --update-env \
    || ${IMPORT_ARCHIVE_ENV}PORT=8788 HOST=0.0.0.0 STATIC_ROOT=/opt/portfolio-tracker-web pm2 start /opt/portfolio-tracker-gateway/portfolio-gateway.mjs --name portfolio-tracker --interpreter node --node-args='--max-old-space-size=96') && \
  pm2 save
"
